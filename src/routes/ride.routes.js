...
// const { emitToUser, pushNewRideToDriver } = require("../socket/socket");
...
router.post("/book", async (req, res) => {
  try {
    ...
    // ✅ No req.io — socket.js uses internal _io
    const emitted = pushNewRideToDriver(driver.userId, ride);
    console.log(
      "📡 Socket emit result:",
      emitted ? "✅ delivered" : "❌ driver not connected",
    );
...
router.patch("/:id/accept", async (req, res) => {
  try {
    ...
    // ✅ No req.io
    const emitted = notifyCustomer(updated.customerId, updated);
    console.log("📡 Customer notified:", emitted ? "✅" : "❌ not connected");
...
router.patch("/:id/arrived", async (req, res) => {
  try {
    ...
    const emitted = notifyCustomer(updated.customerId, updated);
    console.log(
      "✅ Driver arrived — customer notified:",
...
router.patch("/:id/start", async (req, res) => {
  try {
    ...
    const emitted = notifyCustomer(updated.customerId, updated);
    console.log(
      "✅ Ride started — customer notified:",
      emitted ? "✅" : "❌ not connected",
    );
...
router.patch("/:id/complete", async (req, res) => {
  try {
    ...
    const emitted = notifyCustomer(updated.customerId, updated);
    console.log(
      "✅ Ride completed — driver earned ₹" + driverEarning,
      "| customer notified:",
      emitted ? "✅" : "❌ not connected",
    );
...
router.patch("/:id/cancel", async (req, res) => {
  try {
    ...
    // ✅ No req.io — notify the other party based on who cancelled
    if (cancelledBy === "customer") {
      const emitted = notifyDriver(updated.driver.user.id, updated);
      console.log(
        "✅ Ride cancelled by customer — driver notified:",
        emitted ? "✅" : "❌ not connected",
      );
    } else {
      const emitted = notifyCustomer(updated.customerId, updated);
      console.log(
        "✅ Ride cancelled by driver — customer notified:",
        emitted ? "✅" : "❌ not connected",
      );
    }
...
router.patch("/driver/online", async (req, res) => {
  try {
    ...
    // Seed location when going online
    if (isOnline) {
      await prisma.driverLocation.upsert({
        where: { driverId: parseInt(driverId) },
        update: { updatedAt: new Date() },
